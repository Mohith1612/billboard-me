...
`pnpm lint`, `pnpm typecheck` and `pnpm build` cover static/build checks. FOUNDATION-001 and FOUNDATION-002 add targeted shell/SQL commands, but there is no general `test` script, Vitest, Playwright or CI workflow. PR #2 records a historical manual browser/database pass. Never report absent tests as passing.
