...
2. Verify dependencies are human-merged/accepted with evidence. The [backlog](../status/backlog.md) is topologically ordered and the [roadmap](../status/roadmap.md) shows independent lanes. FOUNDATION-001 and FOUNDATION-002 merged in PR #53 and PR #54, and FOUNDATION-003 (#6) awaits review. FOUNDATION-004 (#7) is next once it merges; the behavior suite CI has to run already exists as `pnpm test`.
