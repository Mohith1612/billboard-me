...
| `lib/campaigns` | Preparation, execution, proof decisions and payout eligibility | P0 campaign |
| `lib/storage` | Authorize signed upload/read; validate owner and purpose | P0 private proof |
...
Before live money: isolated preview/test/prod data and secrets, reviewed migrations, tested backup/restore, protected operator access, signed webhooks, reconciliation queue, private evidence, versioned commercial policies, support contact and refund/dispute/payout runbook. A small operator screen shows approvals, unknown orders, creative readiness, overdue proof, disputes and failed settlements.
