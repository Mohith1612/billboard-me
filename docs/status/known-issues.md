...
**Baseline:** 2026-09-15 / `8171a2d`, PR #54 and PR #53 merged. Findings remain open unless marked addressed with verified evidence. [Backlog](backlog.md) links each code to its actual GitHub issue. Repository controls and uncertain deployed configuration are distinguished; no production data was inspected.

| Finding | Evidence / impact | Priority and owner issue | Resolution evidence |
| --- | --- | --- | --- |
| Waitlist deployed access unverified | Repository migration enables RLS and revokes `anon`/`authenticated`; actual Supabase application, exposed schemas, grants and runtime role are not inspected | P0 FOUNDATION-001 and FOUNDATION-002 merged the repository-side controls; deployed proof now sits with FOUNDATION-005 | Disposable fresh/upgrade/route checks pass and the required privileges per connection are documented; operator metadata verification and controlled deployment remain |
| Environment validation | **Addressed in the repository by merged PR #54 / FOUNDATION-002.** Validated public/server env modules; production build rejects a missing or invalid canonical origin; database client created on first use so a static build needs no database; unused Supabase keys removed from the example | FOUNDATION-002 closed; deployed configuration verification remains P0 FOUNDATION-005 | `pnpm test:env` and `pnpm test:client-boundary` verify the contract. Pool size, TLS enforcement and the connection mode a deployment actually uses remain unproven |
| No CI or browser journey | **Behavior suite addressed by FOUNDATION-003:** `pnpm test` runs Vitest unit and integration projects on a disposable PostgreSQL container. No workflow runs it on a pull request, no runtime is pinned, and no Playwright journey exists | P0 FOUNDATION-004 | Passing required checks on a pull request and one production-build browser journey |
