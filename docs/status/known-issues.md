...
**Baseline:** 2026-09-15 / planning merge `b9bea6d` plus the FOUNDATION-001 and FOUNDATION-002 implementations. All below remain open unless a linked issue records verified resolution. [Backlog](backlog.md) links each code to its actual GitHub issue. Repository controls and uncertain deployed configuration are distinguished; no production data was inspected.
...
| Unvalidated environment | **Addressed in the repository by FOUNDATION-002.** Validated public/server env modules; production build rejects a missing or invalid canonical origin; database client created on first use so a static build needs no database; unused Supabase keys removed from the example | P0 FOUNDATION-002, pending founder review/merge | `pnpm test:env` and `pnpm test:client-boundary` pass. Pool size, TLS enforcement and the connection mode a deployment actually uses remain unproven — FOUNDATION-005 |
